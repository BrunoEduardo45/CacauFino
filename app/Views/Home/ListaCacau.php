<?php
    ...
    $produtos = $pdo->prepare("SELECT COUNT(*) FROM cacau WHERE cac_status = 1");
    $produtos->execute();
                ...
                <?php foreach ($list as $values) { ?>
                    <div class="col-lg-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h3><?php echo $values['cac_titulo']; ?></h3>
                                <p class="descricao-limitada"><i><?php echo strip_tags($values['cac_descricao']); ?></i></p>
                                <p><strong>Preço:</strong> R$ <?php echo number_format($values['cac_preco'], 2, ',', '.'); ?></p>
                                <p><strong>Estoque:</strong> <?php echo $values['cac_quantidade'] . ' ' . $values['cac_unidade']; ?></p>
                                
                                <!-- Botão para abrir a modal -->
                                <button class="btn btn-link text-primary p-0 mb-3 w-100 text-center" data-toggle="modal" data-target="#detalhesModal" 
                                        data-id="<?php echo $values['cac_id']; ?>"
                                        data-titulo="<?php echo $values['cac_titulo']; ?>"
                                        data-descricao="<?php echo strip_tags($values['cac_descricao']); ?>"
                                        data-preco="<?php echo number_format($values['cac_preco'], 2, ',', '.'); ?>"
                                        data-quantidade="<?php echo $values['cac_quantidade'] . ' ' . $values['cac_unidade']; ?>">
                                    Ver mais detalhes
                                </button>
                                
                                <!-- Formulário para adicionar ao carrinho -->
                                <form action="" method="POST">
                                    <input type="hidden" name="produto_id" value="<?php echo $values['cac_id']; ?>">
                                    <div class="form-group">
                                        <label for="quantidade_<?php echo $values['cac_id']; ?>">Quantidade</label>
                                        <input type="number" id="quantidade_<?php echo $values['cac_id']; ?>" name="quantidade" class="form-control" min="1" max="<?php echo $values['cac_quantidade']; ?>" value="1" required>
                                    </div>
    ...
    <div class="col-lg-12">
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                <?php
                if ($pagina > 1) {
                    echo "<li class='page-item'><a class='page-link' href='cacaus?pagina=" . ($pagina - 1) . "'>Anterior</a></li>";
                }

                for ($i = 1; $i <= $numPaginas; $i++) {
                    $ativo = ($i == $pagina) ? 'class="active"' : '';
                    echo "<li class='page-item' " . $ativo . "><a class='page-link' href='cacaus?pagina=$i'>" . $i . "</a></li>";
                }

                if ($pagina < $numPaginas) {
                    echo "<li class='page-item'><a class='page-link' href='cacaus?pagina=" . ($pagina + 1) . "'>Próxima</a></li>";
                }
                ?>
            </ul>
        </nav>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            <img id="modalImagem" src="<?php echo $baseUrl ?>/app/public/img/Cacau.png" alt="Cacau" class="img-fluid">
          </div>
          <div class="col-md-6">
